The best logic would be: load the medanya's own users by default, but allow adding extra users from other medanyas via a select. Here's the updated class:
...
```
Then update the view to show the add user form above the repeater:
```php
<x-filament::page>
...
```
The flow is now: the medanya's own users load automatically → you can search and add any user from any other medanya using the select → duplicate prevention is handled → added users are saved normally with updateOrCreate just like the rest.
